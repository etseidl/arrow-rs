...
fn read_column_metadata<'a>(
    ...
    // TODO(ets): if we have an index, we can exit early from this
    // need to examine the options and see which field id's we can exit after
    // if skipping all stats we can exit once we see 12 (statistics)
        ...
        match field_ident.id {
            ...
            3 => {
                if let Some(meta_len) = col_meta_len {
                    ...
                    col_meta_mask =
                        read_column_metadata(&mut meta_prot, &mut col, col_index, options)?;
        ...
        match field_ident.id {
            1 => {
                ...
                if let Some(meta_idx) = index.as_ref() {
                    ...
                    for i in 0..list_ident.size as usize {
                        let col =
                            read_column_chunk(prot, &schema_descr.columns()[i], i, options, None)?;
                        row_group.columns.push(col);
                    }
                }
                mask |= RG_COLUMNS;
...
pub(crate) fn parquet_metadata_from_bytes(
    ...
    // TODO(ets): add option to look for index
    // check for index
    let mut index: Option<MetaIndex> = None;
    ...
    if let Some(options) = options {
        ...
        // only read the meta index if we need to
        if options.skip_any_encoding_stats() {
            index = get_metadata_index(buf)?;
        }
    }
        ...
        match field_ident.id {
            ...
            2 => {
                ...
                if schema_descr.is_some() {
                    match index.as_ref() {
                        Some(meta_idx) => prot.skip_bytes(meta_idx.schema_length as usize)?,
                        _ => prot.skip(field_ident.field_type)?,
                    }
                } else {
                    // read schema and convert to SchemaDescriptor for use when reading row groups
            ...
            4 => {
                ...
                match index.as_ref() {
                    ...
                    _ => {
                        for _ in 0..list_ident.size {
                            rg_vec.push(read_row_group(&mut prot, schema_descr, options, None)?);
                        }
                    }
                }
...
pub(crate) mod tests {
    ...
    pub(crate) fn read_row_group(
        ...
        crate::file::metadata::thrift::read_row_group(&mut reader, &schema_descr, None, None)
    }
    ...
    pub(crate) fn read_column_chunk(
        ...
        let mut reader = ThriftSliceInputProtocol::new(buf);
        crate::file::metadata::thrift::read_column_chunk(&mut reader, &column_descr, 0, None, None)
    }
