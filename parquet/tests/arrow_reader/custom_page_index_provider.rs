...
///
/// Unlike the default `PageIndex`, this provider does not allocate space for
/// num_row_groups X num_columns indexes, but instead uses nested hash maps
...
fn test_read_with_custom_page_index_provider() {
    ...
    assert!(provider.offset_index(2, 0).is_some());
